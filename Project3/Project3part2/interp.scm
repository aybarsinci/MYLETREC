    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (proc-nested-exp (var count name body)
                         (let ((look-val (apply-env env count)))
                             (let ((numval (expval->num look-val)))
                               (proc-val (nested-procedure var body env name numval)))))

        (call-nested-exp (rator rand count)
                         (let ( (proc (expval->proc (value-of rator env)))
                                (arg (cons (value-of rand env) (expval->num (value-of count env)))))
                           (apply-procedure proc arg)))

        

        (letrec-nested-exp (p-name b-var b-count p-body letrec-body)
                           (let ((look-val (apply-env env b-count)))
                             (let ((numval (expval->num look-val)))
                               (value-of letrec-body (extend-env-rec-nested p-name b-var p-body env numval)))))
    ...
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        (nested-procedure (var body saved-env name count)
                          (let ((numb (cdr arg))) 
                            (begin (recursive-displayer name numb)
                                   (value-of body  (extend-env 'count (num-val numb) (extend-env var  (car arg) saved-env)) ) )) )
