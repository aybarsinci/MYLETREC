      ...
      (cases environment env
        ...
        (extend-env (var val saved-env)
          (if (eqv? search-sym var)
              ...
              ; ###### as it normally does.                                        
              ; #####################################################
              (cases expval val  (proc-val (procedurec) (cases proc procedurec 
                                                         (nested-procedure (bvar body env name count) (proc-val (nested-procedure bvar body env var count)) )
                                                         (else val))) 
                (else val))  
        ...
        (extend-env-rec-nested (p-name b-var p-body saved-env count)
                               (if (eqv? search-sym p-name)
                                   (proc-val (nested-procedure b-var p-body env p-name count))
                                   (apply-env saved-env search-sym)))
